'use client'
interface IconProps {
  width?: string
  height?: string
  classStyles?: string
}
export const HashIcon = ({ width, height }: IconProps) => (
  <svg
    height={height}
    width={width}
    version='1.1'
    id='Layer_1'
    viewBox='0 0 512 512'
  >
    <path
      style={{ fill: '#FFD782' }}
      d='M457.127,9.186H54.874c-25.231,0-45.686,20.455-45.686,45.686v402.254
	c0,25.233,20.455,45.686,45.686,45.686h402.254c25.231,0,45.686-20.455,45.686-45.686V54.873
	C502.813,29.641,482.359,9.186,457.127,9.186z'
    />
    <path
      style={{ opacity: 0.1 }}
      d='M70.428,457.127V54.873c0-25.233,20.456-45.687,45.687-45.687H54.874
	c-25.231,0-45.687,20.455-45.687,45.687v402.254c0,25.233,20.455,45.686,45.687,45.686h61.241
	C90.884,502.813,70.428,482.358,70.428,457.127z'
    />
    <path
      d='M457.127,0H148.852c-5.074,0-9.186,4.113-9.186,9.186s4.112,9.186,9.186,9.186h308.274c20.128,0,36.501,16.373,36.501,36.5
	v402.254c0,20.126-16.373,36.501-36.501,36.501h-53.511c-5.074,0-9.186,4.113-9.186,9.186s4.112,9.186,9.186,9.186h53.511
	c30.257,0,54.873-24.616,54.873-54.873V54.873C512.001,24.616,487.384,0,457.127,0z'
    />
    <path
      d='M112.107,0H54.874C24.617,0,0.001,24.616,0.001,54.873v402.254C0.001,487.384,24.617,512,54.874,512h311.998
	c5.074,0,9.186-4.113,9.186-9.186s-4.112-9.186-9.186-9.186H54.874c-20.127-0.001-36.501-16.375-36.501-36.501V54.873
	c0-20.126,16.373-36.501,36.501-36.501h57.235c5.074,0,9.186-4.113,9.186-9.186S117.182,0,112.107,0z'
    />
    <path
      style={{ fill: '#DCE5FA' }}
      d='M216.347,80.038c-12.886,0-23.792,8.427-25.774,21.314l-9.914,58.984h-37.671
	c-13.878,0-25.774,11.895-25.774,25.774s11.895,25.774,25.774,25.774h28.749l-14.871,86.245h-31.723
	c-14.375,0-25.774,11.895-25.774,25.774c0,14.375,11.401,25.774,25.774,25.774h22.801l-8.923,52.045
	c-2.478,16.852,9.418,30.236,25.279,30.236c14.869,0,24.287-10.41,26.27-21.314l10.41-60.967h77.819l-9.418,52.045
	c-2.478,15.365,9.418,30.236,25.774,30.236c12.888,0,23.792-8.923,25.774-21.314l10.409-60.967h37.176
	c14.373,0,25.774-11.895,25.774-25.774c0-14.373-11.401-25.774-25.774-25.774h-28.253l14.869-86.245h31.723
	c13.878,0,25.774-11.895,25.774-25.774c0-14.374-11.895-25.774-25.774-25.774h-22.8l8.426-50.062
	c2.478-15.365-9.418-30.236-25.279-30.236c-13.382,0-24.287,8.427-26.27,21.314l-9.914,58.984h-77.819l8.922-50.062
	C244.105,94.91,232.705,80.038,216.347,80.038z M302.098,211.886l-14.869,86.245h-77.323l14.869-86.245H302.098z'
    />
    <path
      d='M295.158,441.147c-10.312,0-20.072-4.516-26.778-12.389c-6.765-7.943-9.704-18.329-8.065-28.494l0.029-0.174l7.46-41.224
	h-59.071l-9.105,53.328c-2.545,13.996-14.672,28.954-35.325,28.954c-10.247,0-19.856-4.321-26.363-11.856
	c-6.752-7.818-9.595-18.083-8.004-28.902l0.034-0.216l7.082-41.307h-11.906c-19.605,0-34.96-15.357-34.96-34.96
	c0-19.277,15.684-34.96,34.96-34.96h23.984l11.703-67.872H142.99c-18.95,0-34.96-16.01-34.96-34.96
	c0-18.952,16.01-34.961,34.96-34.961h29.899l8.615-51.26c2.663-17.1,16.982-29.038,34.844-29.038c10.132,0,19.405,4.2,26.111,11.827
	c6.92,7.871,10.118,18.356,8.775,28.771l-0.067,0.436l-6.997,39.264h59.079l8.615-51.264c2.665-17.099,17.187-29.033,35.339-29.033
	c10.039,0,19.601,4.467,26.233,12.256c6.803,7.988,9.761,18.424,8.114,28.629l-6.634,39.414h11.938
	c19.277,0,34.96,15.684,34.96,34.961c0,18.95-16.01,34.96-34.96,34.96H362.87l-11.702,67.872h17.347
	c19.604,0,34.96,15.357,34.96,34.96c0,19.277-15.684,34.96-34.96,34.96h-29.423l-9.104,53.328
	C327.31,428.93,312.656,441.147,295.158,441.147z M278.441,403.263c-0.753,4.817,0.675,9.764,3.927,13.581
	c3.257,3.824,7.798,5.929,12.791,5.929c8.421,0,15.445-5.711,16.704-13.578l11.729-68.701h44.926
	c8.991,0,16.588-7.596,16.588-16.588c0-9.301-7.286-16.588-16.588-16.588H329.36l18.037-104.617h39.461
	c8.836,0,16.588-7.752,16.588-16.588c0-8.993-7.596-16.589-16.588-16.589h-33.661l10.229-60.773
	c0.778-4.821-0.668-9.848-3.975-13.73c-3.182-3.737-7.531-5.795-12.247-5.795c-9.037,0-15.946,5.436-17.19,13.524l-0.02,0.126
	l-11.201,66.647h-96.558l10.803-60.62c0.603-5.111-0.986-10.236-4.375-14.092c-3.167-3.602-7.54-5.586-12.313-5.586
	c-8.742,0-15.451,5.436-16.696,13.524l-0.02,0.126l-11.202,66.647h-45.442c-8.837,0-16.588,7.752-16.588,16.589
	c0,8.836,7.752,16.588,16.588,16.588h39.655l-18.039,104.617h-39.46c-8.991,0-16.588,7.596-16.588,16.588
	c0,9.301,7.286,16.588,16.588,16.588h33.696L148.1,403.161c-0.772,5.416,0.557,10.427,3.747,14.12
	c3.06,3.542,7.484,5.493,12.459,5.493c10.173,0,15.985-6.917,17.232-13.771l11.696-68.51h96.567L278.441,403.263z M294.966,307.316
	h-95.967l18.038-104.617h95.966L294.966,307.316z M220.81,288.944h58.68l11.702-67.872h-58.68L220.81,288.944z'
    />
  </svg>
)

export const MailIcon = ({ width, height, classStyles }: IconProps) => (
  <svg
    id='Layer_1'
    xmlns='http://www.w3.org/2000/svg'
    viewBox='0 0 512 512'
    width={width}
    height={height}
    className={classStyles}
  >
    <rect y='81.571' style={{ fill: '#CFDCE5' }} width='512' height='348.857' />
    <path
      style={{ fill: '#BAC6CC' }}
      d='M512,81.571h-14.571L256,292.568L14.558,81.571H0v12.73L185.025,256L0,417.698v12.73h14.571
	L199.595,268.73L256,318.233l56.405-49.503l185.025,161.698H512v-12.73L326.963,256L512,94.302V81.571z M199.595,268.73
	L199.595,268.73L199.595,268.73L199.595,268.73z M312.405,268.73L312.405,268.73L312.405,268.73L312.405,268.73z'
    />
  </svg>
)

export const BBVAIcon = ({ height, width, classStyles }: IconProps) => (
  <svg
    width={width ?? '800px'}
    height={height ?? '800px'}
    viewBox='0 0 48 48'
    id='b'
    xmlns='http://www.w3.org/2000/svg'
    className={`rounded fill-none stroke-white ${classStyles}`}
  >
    <path
      className='c'
      d='m40.5,5.5H7.5c-1.1,0-2,.9-2,2v33c0,1.1.9,2,2,2h33c1.1,0,2-.9,2-2V7.5c0-1.1-.9-2-2-2Z'
    />

    <g>
      <polyline
        className='c'
        points='31.1909 19.7988 27.8429 27.7988 24.4949 19.7988'
      />

      <polyline
        className='c'
        points='31.5332 26.2012 34.8812 18.2012 38.2292 26.2012'
      />

      <g>
        <path
          className='c'
          d='m20.6829,23.7988c1.1046,0,2,.8954,2,2s-.8954,2-2,2h-3.3v-8h3.3c1.1046,0,2,.8954,2,2s-.8954,2-2,2h0Z'
        />

        <line
          className='c'
          x1='20.6829'
          y1='23.7988'
          x2='17.3829'
          y2='23.7988'
        />
      </g>

      <g>
        <path
          className='c'
          d='m13.0708,23.7988c1.1046,0,2,.8954,2,2s-.8954,2-2,2h-3.3v-8h3.3c1.1046,0,2,.8954,2,2s-.8954,2-2,2h0Z'
        />

        <line
          className='c'
          x1='13.0708'
          y1='23.7988'
          x2='9.7708'
          y2='23.7988'
        />
      </g>
    </g>
  </svg>
)

export const MasterCArdIcon = ({ width, height, classStyles }: IconProps) => (
  <svg
    className={`fill-current text-white ${classStyles}`}
    xmlns='http://www.w3.org/2000/svg'
    width={width ?? '30'}
    height={height ?? '30'}
    viewBox='0 0 48 48'
  >
    <path fill='#ff9800' d='M32 10A14 14 0 1 0 32 38A14 14 0 1 0 32 10Z'></path>
    <path fill='#d50000' d='M16 10A14 14 0 1 0 16 38A14 14 0 1 0 16 10Z'></path>
    <path
      fill='#ff3d00'
      d='M18,24c0,4.755,2.376,8.95,6,11.48c3.624-2.53,6-6.725,6-11.48s-2.376-8.95-6-11.48 C20.376,15.05,18,19.245,18,24z'
    ></path>
  </svg>
)

export const NoCameraIcon = ({ width, height, classStyles }: IconProps) => (
  <svg
    className={`${classStyles}`}
    xmlns='http://www.w3.org/2000/svg'
    width={width}
    height={height}
    viewBox='0 0 512.001 512.001'
  >
    <rect
      x='219.389'
      y='122.964'
      style={{ fill: '#F7B239' }}
      width='73.232'
      height='38.446'
    />
    <path
      style={{ fill: '#333333' }}
      d='M292.615,189.807h-73.232c-5.057,0-9.154-4.099-9.154-9.154v-57.689c0-5.056,4.098-9.154,9.154-9.154
	h73.232c5.057,0,9.154,4.099,9.154,9.154v57.69C301.769,185.709,297.672,189.807,292.615,189.807z M228.537,171.498h54.923v-39.38
	h-54.923L228.537,171.498L228.537,171.498z'
    />
    <path
      style={{ fill: '#4D4D4D' }}
      d='M422.602,186.157v164.707c0,13.671-11.083,24.754-24.754,24.754H114.146
	c-13.671,0-24.754-11.083-24.754-24.754V186.157c0-13.671,11.083-24.754,24.754-24.754h283.702
	C411.519,161.404,422.602,172.487,422.602,186.157z'
    />
    <path
      style={{ fill: '#B8E6FF' }}
      d='M422.602,186.157v17.357h-89.713v-42.11h64.96C411.519,161.404,422.602,172.487,422.602,186.157z'
    />
    <circle style={{ fill: '#999999' }} cx='255.995' cy='268.507' r='71.616' />
    <circle style={{ fill: '#E6E6E6' }} cx='255.995' cy='268.507' r='34.574' />
    <path
      style={{ fill: '#808080' }}
      d='M255.999,312.239c-24.113,0-43.729-19.616-43.729-43.728s19.616-43.729,43.729-43.729
	c24.112,0,43.728,19.616,43.728,43.729C299.728,292.623,280.111,312.239,255.999,312.239z M255.999,243.092
	c-14.016,0-25.42,11.403-25.42,25.42c0,14.016,11.404,25.419,25.42,25.419s25.419-11.403,25.419-25.419
	C281.419,254.494,270.017,243.092,255.999,243.092z'
    />
    <circle style={{ fill: '#F7B239' }} cx='168.734' cy='330.171' r='9.154' />
    <path
      style={{ fill: '#F95428' }}
      d='M437.019,74.982C388.667,26.63,324.379,0.001,255.999,0.001S123.332,26.63,74.981,74.982
	C26.628,123.333,0,187.621,0,256.001s26.628,132.667,74.981,181.018C123.332,485.372,187.619,512,256.001,512
	c68.379,0,132.666-26.628,181.02-74.981c48.351-48.351,74.981-112.639,74.981-181.018S485.37,123.333,437.019,74.982z
	 M424.072,87.929c44.895,44.893,69.619,104.583,69.619,168.073c0,60.428-22.411,117.402-63.279,161.465L94.535,81.589
	C138.597,40.723,195.572,18.31,255.999,18.31C319.488,18.31,379.178,43.034,424.072,87.929z M87.928,424.074
	C43.033,379.18,18.309,319.491,18.309,256.001c0-60.428,22.411-117.403,63.279-161.465l335.876,335.876
	c-44.063,40.867-101.038,63.279-161.465,63.279C192.51,493.692,132.82,468.967,87.928,424.074z'
    />
  </svg>
)

export const NoPhoneIcon = ({ width, height, classStyles }: IconProps) => (
  <svg
    className={`${classStyles}`}
    width={width ?? '800px'}
    height={height ?? '800px'}
    viewBox='0 0 72 72'
    xmlns='http://www.w3.org/2000/svg'
  >
    <g id='color'>
      <circle cx='36' cy='36' r='26' fill='#ffffff' />
      <path fill='none' d='m29.48 20h13.52c1.104 0 2 0.8955 2 2v13.52' />
      <path fill='none' d='m42.52 52h-13.52c-1.104 0-2-0.8955-2-2v-13.52' />
      <polyline
        fill='#ffffff'
        stroke-linecap='round'
        stroke-linejoin='round'
        stroke-miterlimit='10'
        stroke-width='2'
        points='27 37.48 27 50 39.52 50'
      />
      <path
        fill='#ffffff'
        stroke-linecap='round'
        stroke-linejoin='round'
        stroke-miterlimit='10'
        stroke-width='2'
        d='m41.52 52h-12.52c-1.104 0-2-0.8955-2-2h12.52l2 2z'
      />
      <polyline
        fill='#ffffff'
        stroke-linecap='round'
        stroke-linejoin='round'
        stroke-miterlimit='10'
        stroke-width='2'
        points='45 34.52 45 22 32.48 22'
      />
      <path
        fill='#ffffff'
        stroke-linecap='round'
        stroke-linejoin='round'
        stroke-miterlimit='10'
        stroke-width='2'
        d='m30.48 20h12.52c1.104 0 2 0.8955 2 2h-12.52l-2-2z'
      />
      <path
        fill='#ea5a47'
        stroke-miterlimit='10'
        d='m36 7c-16.02 0-29 12.98-29 29s12.98 29 29 29 29-12.98 29-29-12.98-29-29-29zm-25.21 29.27c0-5.075 3.417-12.69 6.202-16.49l35.2 35.27c-3.805 2.784-10.93 5.904-16.01 5.904-12.7 0-25.39-11.98-25.39-24.68zm44.87 15.18-35.06-35.13c3.805-2.784 10.77-5.441 15.84-5.441 12.7 0 24.68 12.25 24.68 24.95 0 5.075-2.686 11.81-5.47 15.62z'
      />
    </g>
    <g id='hair' />
    <g id='skin' />
    <g id='skin-shadow' />
    <g id='line'>
      <path
        fill='none'
        stroke='#000000'
        stroke-linecap='round'
        stroke-linejoin='round'
        stroke-miterlimit='10'
        stroke-width='2'
        d='m27 35v15h12.52'
      />
      <path
        fill='none'
        stroke='#000000'
        stroke-linecap='round'
        stroke-linejoin='round'
        stroke-miterlimit='10'
        stroke-width='2'
        d='m44.02 52h-15.02c-1.104 0-2-0.8955-2-2h15.02z'
      />
      <path
        fill='none'
        stroke='#000000'
        stroke-linecap='round'
        stroke-linejoin='round'
        stroke-miterlimit='10'
        stroke-width='2'
        d='m45 36v-14h-12.52'
      />
      <path
        fill='none'
        stroke='#000000'
        stroke-linecap='round'
        stroke-linejoin='round'
        stroke-miterlimit='10'
        stroke-width='2'
        d='m28.98 20h14.02c1.104 0 2 0.8955 2 2h-14.02z'
      />
      <circle
        cx='36'
        cy='36'
        r='30'
        fill='none'
        stroke='#000000'
        stroke-linecap='round'
        stroke-linejoin='round'
        stroke-width='2'
      />
      <path
        fill='none'
        stroke='#000000'
        stroke-linecap='round'
        stroke-linejoin='round'
        stroke-width='2'
        d='m55.66 51.44a24.99 24.99 0 0 0-35.06-35.13z'
      />
      <path
        fill='none'
        stroke='#000000'
        stroke-linecap='round'
        stroke-linejoin='round'
        stroke-width='2'
        d='m16.99 19.77a24.99 24.99 0 0 0 35.2 35.27z'
      />
    </g>
  </svg>
)

export const WeatherIcon = ({ width, height, classStyles }: IconProps) => (
  <svg
    className={`iconify iconify--emojione ${classStyles}`}
    width={width ?? '800px'}
    height={height ?? '800px'}
    viewBox='0 0 64 64'
    xmlns='http://www.w3.org/2000/svg'
    aria-hidden='true'
    role='img'
    preserveAspectRatio='xMidYMid meet'
  >
    <g fill='#ffce31'>
      <path d='M14.7 44.4l4.8-4.9c-1.7-.4-3.3-1-4.7-2l-.1 6.9'></path>

      <path d='M30.3 6.6l-4.8 4.9c1.7.4 3.3 1 4.7 2l.1-6.9'></path>

      <path d='M3.6 33.3l6.9-.1c-.9-1.4-1.6-3-2-4.7l-4.9 4.8'></path>

      <path d='M41.4 17.7l-6.9.1c.9 1.4 1.6 3 2 4.7l4.9-4.8'></path>

      <path d='M3.6 17.7l4.9 4.8c.4-1.7 1-3.3 2-4.7l-6.9-.1'></path>

      <path d='M41.4 33.3l-4.9-4.8c-.4 1.7-1 3.3-2 4.7l6.9.1'></path>

      <path d='M14.7 6.6l.1 6.9c1.4-.9 3-1.6 4.7-2l-4.8-4.9'></path>

      <path d='M30.3 44.4l-.1-6.9c-1.4.9-3 1.6-4.7 2l4.8 4.9'></path>

      <path d='M10.7 33.7L8 40l6.3-2.7c-1.4-1-2.6-2.2-3.6-3.6'></path>

      <path d='M34.3 17.3L37 11l-6.3 2.7c1.4 1 2.6 2.2 3.6 3.6'></path>

      <path d='M8.4 22.9L2 25.5l6.4 2.6c-.1-.8-.2-1.7-.2-2.6c-.1-.9 0-1.7.2-2.6'></path>

      <path d='M36.6 28.1l6.4-2.6l-6.4-2.6c.2.8.2 1.7.2 2.6c0 .9 0 1.7-.2 2.6'></path>

      <path d='M14.3 13.7L8 11l2.7 6.3c1-1.4 2.2-2.6 3.6-3.6'></path>

      <path d='M30.7 37.3L37 40l-2.7-6.3c-1 1.4-2.2 2.6-3.6 3.6'></path>

      <path d='M25.1 11.4L22.5 5l-2.6 6.4c.8-.1 1.7-.2 2.6-.2c.9-.1 1.7 0 2.6.2'></path>

      <path d='M19.9 39.6l2.6 6.4l2.6-6.4c-.8.1-1.7.2-2.6.2c-.9 0-1.7 0-2.6-.2'></path>

      <circle cx='22.5' cy='25.5' r='13'></circle>
    </g>

    <path
      d='M17.2 57.1c-1 0-2-.2-2.9-.5c-3.8-1.3-6.4-4.9-6.4-9c0-2.7 1.2-5.4 3.2-7.2c.5-.5 1.1-.9 1.7-1.2l.6-2.2c1.5-5.5 6.5-9.3 12.1-9.3c.6 0 1.1 0 1.8.1l1.5.3l.2-.5c2.3-4.2 6.6-6.7 11.2-6.7C47.3 20.9 53 26.8 53 34v1.3c.5.2 1.1.5 1.6.8c3.4 2 5.5 5.8 5.5 9.8c0 5.3-3.5 9.8-8.6 11c-.8.2-1.7.3-2.5.3H17.2z'
      fill='#ffffff'
    ></path>

    <path
      d='M40.3 22.8c6 0 10.9 5 10.9 11.2v.9c-2.5.2-4.8 1.2-6.6 2.8c1.3-.8 2.9-1.2 4.6-1.2c.6 0 1.2.1 1.8.2c1 .2 1.9.6 2.8 1.1c2.7 1.6 4.6 4.6 4.6 8.1c0 4.4-3 8.2-7.1 9.1c-.7.2-1.3.2-2.1.2h-32c-.8 0-1.6-.1-2.3-.4c-3-1-5.1-3.9-5.1-7.2c0-2.3 1-4.3 2.6-5.7c.7-.6 1.5-1.1 2.4-1.5c.8-.3 1.6-.4 2.5-.4c2.4 0 4.5 1.2 5.9 3h.1c-1.6-3-4.5-5.1-8-5.4c1.3-4.6 5.4-8 10.3-8c.5 0 1 .1 1.5.1c1 .1 1.9.4 2.8.8c3.3 1.5 5.7 4.7 6.3 8.5v-.1c0-4.3-2.2-8.1-5.4-10.3c1.8-3.5 5.3-5.8 9.5-5.8m0-3.8c-5 0-9.7 2.6-12.4 7h-.3c-.8-.1-1.4-.2-2.1-.2c-6.4 0-12.2 4.4-13.9 10.7l-.5 1.5c-.5.3-.9.6-1.3 1C7.4 41.1 6 44.3 6 47.6c0 4.9 3.1 9.3 7.7 10.8c1.1.4 2.3.6 3.5.6h31.9c1 0 1.9-.1 2.9-.3c5.9-1.4 10-6.7 10-12.8c0-4.7-2.5-9.1-6.5-11.4l-.6-.3v-.1c0-8.4-6.5-15.1-14.6-15.1z'
      fill='#75d6ff'
    ></path>
  </svg>
)

export const MapIcon = ({ width, height, classStyles }: IconProps) => (
  <svg
    className={`icon ${classStyles}`}
    width={width ?? '800px'}
    height={height ?? '800px'}
    viewBox='0 0 1024 1024'
    version='1.1'
    xmlns='http://www.w3.org/2000/svg'
  >
    <path
      d='M318.7 233.66666667H165.1c-22.8 0-41.3 18.5-41.3 41.3v513.3c0 22.8 18.5 41.3 41.3 41.3h753.7c22.8 0 41.3-18.5 41.3-41.3V274.96666667c0-22.8-18.5-41.3-41.3-41.3H318.7z'
      fill='#A7B8C6'
    />
    <path
      d='M292.6 210.8H144c-22.1 0-39.9 17.7-39.9 39.6V743c0 21.9 17.9 39.6 39.9 39.6h729.1c22.1 0 39.9-17.7 39.9-39.6V250.4c0-21.9-17.9-39.6-39.9-39.6H292.6z'
      fill='#9ED09E'
    />
    <path
      d='M372.2 675c-9.7 8.2-10.9 22.9-2.7 32.6 8.2 9.7 22.9 10.9 32.6 2.7l498.1-421.9v-60.6L372.2 675zM547.1 274.5c-8.2-9.7-22.9-10.9-32.6-2.7L104.1 619.4V680l440.3-372.9c9.7-8.3 10.9-22.9 2.7-32.6z'
      fill='#BBDBBB'
    />
    <path
      d='M498.6 484.6m-188.9 0a188.9 188.9 0 1 0 377.8 0 188.9 188.9 0 1 0-377.8 0Z'
      fill='#FFFFFF'
    />
    <path
      d='M706.6 506.7H284.7c-4 0-7.3-3.3-7.3-7.3v-29.5c0-4 3.3-7.3 7.3-7.3h421.9c4 0 7.3 3.3 7.3 7.3v29.5c0 4-3.3 7.3-7.3 7.3z'
      fill='#FFFFFF'
    />
    <path
      d='M473.5 695.6V273.7c0-4 3.3-7.3 7.3-7.3h29.5c4 0 7.3 3.3 7.3 7.3v421.9c0 4-3.3 7.3-7.3 7.3h-29.5c-4 0-7.3-3.3-7.3-7.3z'
      fill='#FFFFFF'
    />
    <path
      d='M498.6 484.6m-152.9 0a152.9 152.9 0 1 0 305.8 0 152.9 152.9 0 1 0-305.8 0Z'
      fill='#4DA6D8'
    />
    <path
      d='M498.6 484.6m-76.6 0a76.6 76.6 0 1 0 153.2 0 76.6 76.6 0 1 0-153.2 0Z'
      fill='#9ACDE0'
    />
    <path
      d='M498.6 484.6m-40.8 0a40.8 40.8 0 1 0 81.6 0 40.8 40.8 0 1 0-81.6 0Z'
      fill='#FFFFFF'
    />
    <path
      d='M549.7 395.2c5.4-14.4 2.8-28.8-5.8-32-8.5-3.2-19.9 6-25.3 20.4l-33.7 90.5 31.1 11.6 33.7-90.5z'
      fill='#BA5955'
    />
    <path
      d='M484.9 474.1l-37.3 100c-5.4 14.4-2.8 28.8 5.8 32 8.5 3.2 19.9-6 25.3-20.4l37.3-100-31.1-11.6z'
      fill='#FFFFFF'
    />
    <path
      d='M872.4 796.1H118.8c-30.2 0-54.9-24.6-54.9-54.8V228c0-30.2 24.6-54.9 54.9-54.9h753.6c30.2 0 54.8 24.6 54.8 54.9v513.3c0.1 30.2-24.5 54.8-54.8 54.8zM118.8 200.2c-15.3 0-27.7 12.4-27.7 27.7v513.3c0 15.3 12.4 27.7 27.7 27.7h753.6c15.3 0 27.7-12.4 27.7-27.7V228c0-15.3-12.4-27.7-27.7-27.7H118.8z'
      fill='#3E3A39'
    />
  </svg>
)

export const SunIcon = ({ width, height, classStyles }: IconProps) => (
  <svg
    className={`iconify iconify--noto ${classStyles}`}
    width={width ?? '800px'}
    height={height ?? '800px'}
    viewBox='0 0 128 128'
    xmlns='http://www.w3.org/2000/svg'
    aria-hidden='true'
    role='img'
    preserveAspectRatio='xMidYMid meet'
  >
    <path
      d='M37.41 41.95c-9.71 12.48-9.54 34.65 2.87 45.64c14.09 12.47 33.92 12.34 46.39.87c14.95-13.76 14.09-36.66.87-49.63c-13.29-13.04-37.04-13.72-50.13 3.12z'
      fill='#fcc11a'
    ></path>
    <path
      d='M53 37.67c-3.84-1.7-8.04 2.93-9.87 6.09c-1.83 3.17-3.53 9.38.37 10.97c3.9 1.58 6.7-1.1 9.51-5.73c2.79-4.63 4.38-9.38-.01-11.33z'
      fill='#fee269'
    ></path>
    <path
      d='M63 20.27c-.93 1.74-.62 3.08 1.23 3.52c1.85.44 13.36 2.31 14.33 2.37c1.41.09 1.93-.97 1.76-2.2c-.18-1.23-2.99-18.46-3.25-20.04S75.14.76 73.55 2.87S63.7 18.96 63 20.27z'
      fill='#ffa722'
    ></path>
    <path
      d='M92.8 32.23c-1.81.56-1.76 1.67-.79 3.08c.97 1.41 7.65 11.6 8.26 12.31c.62.7 1.67.88 2.55-.18c.88-1.05 11.86-16.45 12.66-17.41c1.32-1.58.53-3.25-1.49-2.73c-1.54.41-20.05 4.58-21.19 4.93z'
      fill='#ffa722'
    ></path>
    <path
      d='M106.6 61.86c-1.3-.74-2.99-.53-3.43 1.14c-.44 1.67-2.37 13.8-2.55 14.86s.62 2.11 1.93 1.85s19.45-2.95 20.66-3.25c2.11-.53 2.81-2.64.62-4.22c-1.42-1.03-16-9.68-17.23-10.38z'
      fill='#ffa722'
    ></path>
    <path
      d='M92.09 90.6c1.4-.75 2.64-.18 2.99 1.41c.35 1.58 4.22 17.76 4.84 20.75c.31 1.5-1.41 2.73-2.81 1.85c-1.41-.88-16.69-11.53-17.67-12.4c-1.41-1.23-.43-2.51.26-3.16c1.4-1.33 11.07-7.74 12.39-8.45z'
      fill='#ffa722'
    ></path>
    <path
      d='M49.54 99.48c-1.77-.17-2.29 1.41-2.02 2.81c.26 1.41 2.9 19.24 3.08 20.57c.26 1.93 2.29 2.73 3.6.79s10.35-16.4 11.08-17.76c1.32-2.46.35-2.99-.97-3.6c-1.31-.61-12.92-2.63-14.77-2.81z'
      fill='#ffa722'
    ></path>
    <path
      d='M24.23 79c1.23-2.02 2.81-1.49 3.96.44c.78 1.32 7.38 10.2 8 11.16c.62.97.88 2.81-1.05 3.25c-1.95.45-17.68 4.58-20.14 5.02c-2.46.44-3.87-1.49-2.29-3.6c.92-1.24 10.82-15.12 11.52-16.27z'
      fill='#ffa722'
    ></path>
    <path
      d='M20.89 63.7c2.25 1 3.31.64 3.78-.97c.62-2.11 2.46-11.78 2.55-13.98c.06-1.43-.53-2.81-2.73-2.46S6.47 48.85 4.45 49.55c-2.35.82-2.18 3.4-.62 4.22c1.85.97 15.47 9.23 17.06 9.93z'
      fill='#ffa722'
    ></path>
    <path
      d='M48.23 26.78c1.27-1.01.88-2.46-.26-3.25c-1.14-.79-15.26-11-17.05-12.4c-1.58-1.23-3.52-.79-2.99 2.02c.38 2.02 4.88 19.7 5.19 20.92c.35 1.41 1.41 2.11 2.64 1.23c1.21-.87 11.15-7.46 12.47-8.52z'
      fill='#ffa722'
    ></path>
  </svg>
)

export const SunCloudsIcon = ({ width, height, classStyles }: IconProps) => (
  <svg
    className={`iconify iconify--noto ${classStyles}`}
    width={width ?? '800px'}
    height={height ?? '800px'}
    viewBox='0 0 128 128'
    xmlns='http://www.w3.org/2000/svg'
    aria-hidden='true'
    role='img'
    preserveAspectRatio='xMidYMid meet'
  >
    <path
      d='M29.53 45.25c-7.14 9.17-7.02 25.48 2.11 33.55c10.36 9.17 24.93 9.08 34.1.64c10.99-10.11 10.36-26.95.64-36.48c-9.77-9.59-27.23-10.09-36.85 2.29z'
      fill='#fcc11a'
    ></path>
    <path
      d='M40.99 42.1c-2.82-1.25-5.91 2.15-7.26 4.48s-2.6 6.9.27 8.06s4.93-.81 6.99-4.21c2.06-3.4 3.22-6.89 0-8.33z'
      fill='#fee269'
    ></path>
    <path
      d='M48.34 29.31c-.68 1.28-.45 2.26.9 2.59c1.36.32 9.82 1.7 10.53 1.74c1.03.06 1.42-.71 1.29-1.62c-.13-.9-2.2-13.57-2.39-14.74c-.19-1.16-1.42-2.33-2.59-.78c-1.15 1.57-7.23 11.84-7.74 12.81z'
      fill='#ffa722'
    ></path>
    <path
      d='M70.25 38.1c-1.33.41-1.29 1.23-.58 2.26c.71 1.03 5.62 8.53 6.08 9.05s1.23.65 1.87-.13s8.72-12.09 9.31-12.8c.97-1.16.39-2.39-1.1-2c-1.14.3-14.74 3.37-15.58 3.62z'
      fill='#ffa722'
    ></path>
    <path
      d='M19.84 72.49c.9-1.49 2.07-1.1 2.91.32c.58.97 5.43 7.5 5.88 8.21c.45.71.65 2.07-.78 2.39c-1.42.32-12.99 3.36-14.8 3.68s-2.84-1.1-1.68-2.65c.68-.9 7.95-11.11 8.47-11.95z'
      fill='#ffa722'
    ></path>
    <path
      d='M17.38 61.24c1.65.74 2.43.47 2.78-.71c.45-1.55 1.81-8.66 1.87-10.28c.04-1.05-.39-2.07-2-1.81c-1.62.26-13.25 1.87-14.74 2.39c-1.73.6-1.6 2.5-.45 3.1c1.36.72 11.38 6.79 12.54 7.31z'
      fill='#ffa722'
    ></path>
    <path
      d='M37.48 34.1c.93-.74.65-1.81-.19-2.39c-.84-.58-11.22-8.09-12.54-9.11c-1.16-.9-2.59-.58-2.2 1.49c.28 1.48 3.59 14.48 3.81 15.38c.26 1.03 1.03 1.55 1.94.9c.91-.65 8.21-5.5 9.18-6.27z'
      fill='#ffa722'
    ></path>
    <path
      d='M109.13 81.18s1.45-28.45-21.58-34s-29.74 16.79-29.74 16.79s-7.68-1.79-14.8 4.13c-5.59 4.65-6.38 11.63-6.38 11.63s-7.35-1.12-13.47 4.82c-4.43 4.3-4 9.82-4 9.82l8.92 7.84l90.65-2.45l4.77-5.12s.76-5.68-4.54-9.95c-5.3-4.28-9.83-3.51-9.83-3.51z'
      fill='#e2ebee'
    ></path>
    <path
      d='M40.64 92.23s3.61 4.08 15.06 3.81s14.46-3.27 14.46-3.27s5.02 5.27 19.53 4.15s18.68-4.18 18.68-4.18s3.54 2.53 8.02 2.6c4.47.07 7.11-.71 7.11-.71s-.47 3.56-2.03 5.84c-1.56 2.28-4.16 4.02-8.27 3.99c-4.12-.03-16.64.08-29.35.06c-12.71-.02-49.03.19-54.4-.04c-4.09-.17-7.06-2.58-8.43-4.62c-2.19-3.26-1.83-6.7-1.83-6.7s4.12 3.23 9.68 2.99c7.6-.32 11.77-3.92 11.77-3.92z'
      fill='#b9ced3'
    ></path>
  </svg>
)

export const CloudsIcon = ({ width, height, classStyles }: IconProps) => (
  <svg
    className={`iconify iconify--noto ${classStyles}`}
    width={width ?? '800px'}
    height={height ?? '800px'}
    version='1.1'
    id='Layer_1'
    xmlns='http://www.w3.org/2000/svg'
    viewBox='0 0 511.985 511.985'
    xmlSpace='preserve'
  >
    <path
      fill='#AAB2BC'
      d='M382.645,168.665c-37.28,0-70.856,15.781-94.466,41.015c-12.64-14.562-31.296-23.772-52.092-23.772
	c-38.101,0-68.982,30.882-68.982,68.975c0,0.578,0.016,1.148,0.023,1.719c-39.358,7.984-68.99,42.78-68.99,84.497
	c0,47.607,38.601,86.217,86.216,86.217h198.291c71.435,0,129.34-57.89,129.34-129.324
	C511.985,226.562,454.08,168.665,382.645,168.665z'
    />
    <path
      fill='#949DA8'
      d='M462.814,196.501c17.422,22.038,27.828,49.881,27.828,80.162
	c0,71.419-57.904,129.323-129.325,129.323H163.019c-18.358,0-35.374-5.75-49.357-15.531c15.585,22.281,41.437,36.859,70.692,36.859
	h198.291c71.435,0,129.34-57.89,129.34-129.324C511.985,256.843,492.767,220.187,462.814,196.501z'
    />
    <path
      fill='#D7DCE2'
      d='M284.523,84.668c-37.288,0-70.872,15.773-94.481,41.015c-12.64-14.563-31.295-23.772-52.1-23.772
	c-38.093,0-68.975,30.882-68.975,68.974c0,0.57,0.008,1.148,0.023,1.719C29.625,180.588,0,215.384,0,257.102
	c0,47.608,38.601,86.216,86.217,86.216h198.306c71.42,0,129.31-57.904,129.31-129.324
	C413.833,142.565,355.943,84.668,284.523,84.668z'
    />
    <path
      fill='#CCD1D9'
      d='M364.677,112.503c17.422,22.031,27.844,49.881,27.844,80.154c0,71.428-57.904,129.332-129.34,129.332
	H64.882c-18.358,0-35.373-5.75-49.357-15.53c15.586,22.279,41.437,36.857,70.693,36.857h198.305
	c71.42,0,129.31-57.904,129.31-129.324C413.833,172.845,394.614,136.19,364.677,112.503z'
    />
  </svg>
)

export const GoogleMapsIcon = ({ width, height, classStyles }: IconProps) => (
  <svg
    className={`${classStyles}`}
    width={width ?? '16.75'}
    height={height ?? '24'}
    xmlns='http://www.w3.org/2000/svg'
    viewBox='0 0 256 367'
  >
    <path
      fill='#34A853'
      d='M70.585 271.865a370.712 370.712 0 0 1 28.911 42.642c7.374 13.982 10.448 23.463 15.837 40.31c3.305 9.308 6.292 12.086 12.714 12.086c6.998 0 10.173-4.726 12.626-12.035c5.094-15.91 9.091-28.052 15.397-39.525c12.374-22.15 27.75-41.833 42.858-60.75c4.09-5.354 30.534-36.545 42.439-61.156c0 0 14.632-27.035 14.632-64.792c0-35.318-14.43-59.813-14.43-59.813l-41.545 11.126l-25.23 66.451l-6.242 9.163l-1.248 1.66l-1.66 2.078l-2.914 3.319l-4.164 4.163l-22.467 18.304l-56.17 32.432z'
    />
    <path
      fill='#FBBC04'
      d='M12.612 188.892c13.709 31.313 40.145 58.839 58.031 82.995l95.001-112.534s-13.384 17.504-37.662 17.504c-27.043 0-48.89-21.595-48.89-48.825c0-18.673 11.234-31.501 11.234-31.501l-64.489 17.28z'
    />
    <path
      fill='#4285F4'
      d='M166.705 5.787c31.552 10.173 58.558 31.53 74.893 63.023l-75.925 90.478s11.234-13.06 11.234-31.617c0-27.864-23.463-48.68-48.81-48.68c-23.969 0-37.735 17.475-37.735 17.475v-57z'
    />
    <path
      fill='#1A73E8'
      d='M30.015 45.765C48.86 23.218 82.02 0 127.736 0c22.18 0 38.89 5.823 38.89 5.823L90.29 96.516H36.205z'
    />
    <path
      fill='#EA4335'
      d='M12.612 188.892S0 164.194 0 128.414c0-33.817 13.146-63.377 30.015-82.649l60.318 50.759z'
    />
  </svg>
)

export const UberIcon = ({ width, height, classStyles }: IconProps) => (
  <svg
    className={`${classStyles}`}
    width={width ?? '24'}
    height={height ?? '24'}
    xmlns='http://www.w3.org/2000/svg'
    viewBox='0 0 32 32'
  >
    <path
      fill='white'
      d='M15.958.01C7.744.005.864 6.229.047 14.401h11.115V12a.8.8 0 0 1 .797-.797h8c.443 0 .797.359.797.802V20a.797.797 0 0 1-.797.797h-8a.797.797 0 0 1-.797-.797v-2.401H.047c.885 8.792 8.724 15.198 17.51 14.313c8.792-.88 15.198-8.724 14.313-17.51C31.052 6.23 24.172.011 15.959.011z'
    />
  </svg>
)

export const MapsIcon = ({ width, height, classStyles }: IconProps) => (
  <svg
    className={`${classStyles}`}
    width={width ?? '24'}
    height={height ?? '24'}
    xmlns='http://www.w3.org/2000/svg'
    viewBox='0 0 24 24'
  >
    <path
      fill='#888'
      d='M17.05 20.28c-.98.95-2.05.8-3.08.35c-1.09-.46-2.09-.48-3.24 0c-1.44.62-2.2.44-3.06-.35C2.79 15.25 3.51 7.59 9.05 7.31c1.35.07 2.29.74 3.08.8c1.18-.24 2.31-.93 3.57-.84c1.51.12 2.65.72 3.4 1.8c-3.12 1.87-2.38 5.98.48 7.13c-.57 1.5-1.31 2.99-2.54 4.09zM12.03 7.25c-.15-2.23 1.66-4.07 3.74-4.25c.29 2.58-2.34 4.5-3.74 4.25'
    />
  </svg>
)

export const WazeIcon = ({ width, height, classStyles }: IconProps) => (
  <svg
    className={`${classStyles}`}
    width={width ?? '24'}
    height={height ?? '24'}
    xmlns='http://www.w3.org/2000/svg'
    viewBox='0 0 24 24'
  >
    <path
      fill='currentColor'
      d='M20.54 6.63c.69.94 1.15 2.04 1.35 3.19c.21 1.25.11 2.52-.31 3.72a7.349 7.349 0 0 1-2 3.06a9.1 9.1 0 0 1-2.26 1.58c.41 1.07-.13 2.27-1.2 2.68c-.24.09-.49.14-.74.14a2.08 2.08 0 0 1-2.07-2h-3.07c-.11 1.14-1.13 2-2.27 1.87c-1.06-.1-1.86-.98-1.88-2.04c.01-.19.04-.39.1-.57a8.37 8.37 0 0 1-4-2.85c-.33-.44-.23-1.07.23-1.41c.18-.14.4-.22.63-.22c.72 0 1-.25 1.17-.63c.24-.72.38-1.47.39-2.23c.03-.53.09-1.05.17-1.57A7.307 7.307 0 0 1 7.5 5c1.66-1.3 3.69-2 5.79-2c1.43 0 2.84.35 4.11 1a8.67 8.67 0 0 1 3.14 2.63m-3.82 10.68c1.78-.81 3.18-2.27 3.87-4.1c1.62-4.94-2.59-9.16-7.3-9.16c-.35 0-.71.02-1.06.07C9.36 4.5 6.4 6.5 5.81 9.5c-.38 2 .19 5.29-2.76 5.29C4 16 5.32 16.93 6.81 17.37c.85-.76 2.16-.68 2.93.18c.11.12.2.25.26.39h3.55c.52-1.02 1.78-1.44 2.8-.9c.15.08.25.17.37.27m-5.75-7c-.58.03-1.09-.41-1.12-1c-.03-.58.42-1.08 1-1.12c.58-.03 1.09.42 1.12 1.06a.999.999 0 0 1-.97 1.04zm4.69 0c-.58.03-1.09-.41-1.12-1c-.04-.58.42-1.08 1-1.12c.58-.03 1.09.42 1.12 1.06c.02.55-.41 1.02-1 1.04zm-5.95 1.76c-.06-.28.13-.57.41-.62c.28-.05.56.13.62.41a2.501 2.501 0 0 0 2.58 1.74c1.14.06 2.18-.64 2.57-1.72c.14-.26.46-.38.71-.23c.18.1.29.27.29.47c-.19.71-.63 1.33-1.23 1.76c-.69.48-1.5.75-2.34.76h-.11c-1.63.07-3.1-1-3.53-2.58z'
    />
  </svg>
)

export const ChevronDown = ({ width, height, classStyles }: IconProps) => (
  <svg
    className={`${classStyles} stroke-current`}
    width={width ?? '800px'}
    height={height ?? '800px'}
    viewBox='0 0 24 24'
    fill='none'
    xmlns='http://www.w3.org/2000/svg'
  >
    <path
      d='M6 9L12 15L18 9'
      strokeWidth='2'
      strokeLinecap='round'
      strokeLinejoin='round'
    />
  </svg>
)

export const RightArrowIcon = ({ width, height, classStyles }: IconProps) => (
  <svg
    className={`${classStyles}`}
    width={width ?? '15'}
    height={height ?? '15'}
    viewBox='0 0 15 15'
    fill='none'
    xmlns='http://www.w3.org/2000/svg'
  >
    <path
      d='M8.14645 3.14645C8.34171 2.95118 8.65829 2.95118 8.85355 3.14645L12.8536 7.14645C13.0488 7.34171 13.0488 7.65829 12.8536 7.85355L8.85355 11.8536C8.65829 12.0488 8.34171 12.0488 8.14645 11.8536C7.95118 11.6583 7.95118 11.3417 8.14645 11.1464L11.2929 8H2.5C2.22386 8 2 7.77614 2 7.5C2 7.22386 2.22386 7 2.5 7H11.2929L8.14645 3.85355C7.95118 3.65829 7.95118 3.34171 8.14645 3.14645Z'
      fill='currentColor'
      fillRule='evenodd'
      clipRule='evenodd'
    ></path>
  </svg>
)
